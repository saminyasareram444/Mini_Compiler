...
%token NUM VAR IF ELIF ELSE MAIN INT FLOAT CHAR COM START END SWITCH CASE DEFAULT BREAK LOOP PF SIN COS TAN LOG BIGMOD LOG10
%nonassoc IFX
%nonassoc ELSE
%nonassoc SWITCH
%nonassoc CASE
...
statement: ';'			
	...
	| LOOP '(' expression '<' expression ';' expression '-''-' ')' START statement END {
	                                int i;
	                                for(i=$5 ; i>$3 ; i--) {printf("value of the loop: %d expression value: %d\n", i,$12);}									
				               }
	| LOOP '(' expression '>' expression ';' expression '+''+' ')' START statement END {
	                                int i;
	                                for(i=$5 ; i<$3 ; i--) {printf("value of the loop: %d expression value: %d\n", i,$12);}									
